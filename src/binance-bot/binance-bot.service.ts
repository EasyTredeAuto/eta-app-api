...
export class BinanceBotService {
  ...
  async create(user, body: CreateBotDto) {
    const botExist = await this.botRepository.findOne({
      where: { name: body.name, user },
    })
    if (botExist)
      throw new BadRequestException('Bot already registered with name')
    const bot = {
      name: body.name,
      asset: body.asset,
      currency: body.currency,
      timeFleam: body.timeFleam,
      amount: body.amount,
