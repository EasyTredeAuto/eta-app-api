...
export class UserController {
  ...
  @Auth()
  @Post()
  async createOne(@Body() body: CreateUserDto, @Request() request) {
    const { roles } = request.user.data
    if (roles !== AppRoles.ADMIN)
      throw new BadRequestException('Forbidden resource')
  ...
  @Auth()
  @Put()
  async updateOne(@Body() dto: EditUserDto, @Request() request) {
  ...
  async deleteMany(@Param('ids') ids: string, @Request() request) {
    const { roles } = request.user.data
    if (roles !== AppRoles.ADMIN)
      throw new BadRequestException('Forbidden resource')
